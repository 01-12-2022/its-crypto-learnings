...
export default function XorCipher() {
  ...
  const encrypted = xorEncrypt(text, key || "A");
  const decrypted = xorEncrypt(encrypted, key || "A");
    ...
    <div className="lesson-card">
      ...
      <div className="xor-table">
        <div className="xor-row header">
          ...
          <span>XOR-Ergebnis (Hex)</span>
        </div>
        {text.split("").map((char, i) => {
          const keyChar = (key || "A")[i % (key || "A").length];
      ...
      {showCode && (
        <SyntaxHighlighter language="typescript" style={oneDark} className="code-block">
          {implCode}
        </SyntaxHighlighter>
      )}
    </div>
